...
def bytes_to_bits(data):
    return [bit for byte in data for bit in [(byte >> i) & 1 for i in range(7, -1, -1)]]
...
def encrypt(plain, key):
    ...
    for i in range(0, len(plain), 8):
        block = bytes_to_bits(plain[i:i + 8])
        encrypted = bits_to_bytes(des_block(block, keys))
        cipher += encrypted
    return cipher
